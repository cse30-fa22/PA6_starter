...
int
strtoDate(char *date, time_t *ltime)
{
    ...
    if (sscanf(date,
        ...
        fprintf(stderr, "%s: strtoDate() date format error %s\n", argv0, date);
        return -1;
    }
    ...
    *ltime = mktime(&tmd);
    if (errno != 0) {
        fprintf(stderr, "%s: strtoDate() date conversion failed on %s\n", argv0, date);
        return -1;
    }
    return 0;
}
...
int
strtosumid(char *summ, unsigned long long *summid)
{
    char *endptr;
