 ...
 */
 ...
 readfines(char *finenm)
 {
    ...
    while (getline(&buf, &bufsz, fp) > 0) {
         ...
         */
        if (token(buf, FDELIM, FCOLS, coltab, linecnt, argv0) != 0) {
            retval = -1;
            break;
        }
 ...
 */
 ...
 readtickets(char *datanm, int fromcmd)
 {
    ...
    while (getline(&buf, &bufsz, fp) > 0) {
         ...
         * find the fields in the table
         */
        if (token(buf, DDELIM, DCOLS, coltab, linecnt, argv0) != 0)
            continue;
        ...
        if (insertticket(coltab[SUMM_IN], coltab[PLATE_IN], coltab[STATE_IN],
